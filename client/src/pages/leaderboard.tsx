  ...
  if (!leaderboardData && !isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center flex-col p-6">
        <div className="text-red-600 font-semibold text-xl mb-4">Error Loading Leaderboard</div>
        <div className="text-gray-600 max-w-md text-center mb-4">
          Unable to load the leaderboard data. Please try refreshing.
        </div>
        <Button
          onClick={() => {
            queryClient.invalidateQueries({ queryKey: ['/api/results'] });
            queryClient.invalidateQueries({ queryKey: ['/api/users'] });
            toast({ title: "Refreshing leaderboard", description: "Attempting to reload data..." });
          }}
          variant="outline"
          className="mt-2"
        >
  ...
  return (
    ...
    <div className="relative flex items-center justify-center px-4 py-12 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto w-full">
        ...
        <h1 className="text-3xl font-bold text-gray-800 mb-2">Competition Results</h1>
        ...
        <CardContent className="p-6">
          <h2 className="text-xl font-bold text-gray-800 mb-6 text-center">Leaderboard</h2>
          <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
            <tr>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">No.</th> {/* Row number column */}
            ...
            {sortedResults.slice(0, 100).map((result: Result, index: number) => {
              ...
              let actualRank = 1; // Start from rank 1
